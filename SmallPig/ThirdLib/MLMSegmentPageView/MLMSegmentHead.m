...
- (void)createBtn:(NSArray *)titlesArr addScroll:(UIScrollView*)scroll startX:(CGFloat)start_x start_index:(NSInteger)start_index {
    ...
    scroll.contentSize = CGSizeMake(MAX(SCROLL_WIDTH, sum_width), SCROLL_HEIGHT);
//    scroll.contentOffset = CGPointMake(MAX(SCROLL_WIDTH, sum_width), 0);
}
...
- (UIView *)slideView {
    ...
    UIImageView *slide = [[UIImageView alloc] initWithFrame:CGRectMake(0, (SCROLL_HEIGHT-_slideHeight)/2+4, slide_w*_slideScale, _slideHeight-8)];
...
//    slide.contentMode = UIViewContentModeScaleAspectFill;
    UIImage *img = [UIImage imageNamed:@"dh_r4_c1"];
    // 左端盖宽度
    NSInteger leftCapWidth = img.size.width * 0.5f;
    // 顶端盖高度
    NSInteger topCapHeight = img.size.height * 0.8f;
    // 重新赋值
...
- (void)setSelectIndex:(NSInteger)index {
    ...
    if (self.selectedIndex2) {
        self.selectedIndex2(currentIndex);
    }
}
...
- (void)changeBtnFrom:(NSInteger)from to:(NSInteger)to {
    ...
    if (slideView) {
        ...
        if (to ==0) {
            ...
            [(UIImageView*)slideView setImage:[UIImage imageNamed:@"dh_r1_c2"]];
        }else{
          [(UIImageView*)slideView setImage:[UIImage imageNamed:@"dh_r4_c1"]];
        }
    }
}

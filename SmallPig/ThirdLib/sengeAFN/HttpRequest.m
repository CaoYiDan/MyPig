...
- (void)httpRequestPOST:(NSString *)string parameters:(id)parmeters progress:(ProgressBlock)progress sucess:(SucessBlock)sucess failure:(FailureBlock)failure    {
    //GMT时间
    NSString * currentData=[self getData];
    
    NSString *jsonContent = [self convertToJsonData:parmeters];
   
    //将json字符串 MD5_base64加密
    NSString *contentMD5 = [self md5_base64:jsonContent];

    //将GMT与x-date HMAC_md5加密
    NSString *str= [NSString stringWithFormat:@"x-date: %@\nContent-MD5: %@",currentData,contentMD5];
    NSString*hmac_md5Str=[self hmac:str withKey:key];
    
    //合成Authorization 的value
    NSString * str2 = [NSString stringWithFormat:@"hmac username=\"xzbj\", algorithm=\"hmac-sha1\", headers=\"x-date Content-MD5\", signature=\"%@\"",hmac_md5Str];
    
    self.manager.requestSerializer=[AFJSONRequestSerializer serializer];
    [self.manager.requestSerializer setValue:currentData        forHTTPHeaderField:@"x-date"];
    [self.manager.requestSerializer setValue:contentMD5      forHTTPHeaderField:@"Content-MD5"];
    [self.manager.requestSerializer setValue:@"application/json"  forHTTPHeaderField:@"Content-Type"];
    [self.manager.requestSerializer setValue:str2  forHTTPHeaderField:@"Authorization"];

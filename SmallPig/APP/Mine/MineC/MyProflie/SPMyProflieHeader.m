...
@implementation SPMyProflieHeader
{
...
-(void)sTop{
    ...
    _name = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_W/2-nameWid/2, 0, nameWid, 40)];
...
-(void)sBottom{
    ...
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(kMargin, CGRectGetMaxY(_headerImg.frame)+30, SCREEN_W-2*kMargin, 1)];
    line.backgroundColor = [UIColor lightGrayColor];
    [self addSubview:line];
...
-(void)setUser:(SPUser *)user{
    ...
    [_headerImg sd_setImageWithURL:[NSURL URLWithString:user.avatar] placeholderImage:nil];
    ...
    //年龄和星座
    _ageAndConstellation.text = [NSString stringWithFormat:@"%@ | %@",user.birthday,user.zodiac];
...
//    }
    ...
    //点赞
    [_zan setTitle:@"12" forState:0];
    
    //签名
    _signTextFiled.text = user.signature;
    
    //兴趣
    [_insterest setTitle:user.haunt forState:0];
}

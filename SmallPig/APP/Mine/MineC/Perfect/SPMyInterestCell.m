...
-(void)createUI{
    ...
    [_textForTag mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.offset(50);
        make.size.mas_offset(CGSizeMake(tagWid, 25));
        make.left.offset(0);
    }];
    ...
    [self.contentView addSubview:_textForSkill];
...
-(void)setModel:(SPKungFuModel *)model{
    ...
    if ([model.rootType  isEqualToString:@"SKILL"]) {//武功类型
        ...
        [_imgForTag sd_setImageWithURL:[NSURL URLWithString:model.imgUrl]];

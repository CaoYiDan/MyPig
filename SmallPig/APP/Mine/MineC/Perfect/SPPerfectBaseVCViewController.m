...
- (void)viewDidLoad {
    ...
    if (self.formMyCenter) {
        ...
        //保存
        UIButton *save = [[UIButton alloc]initWithFrame:CGRectMake(SCREEN_W-70 , 25, 70, 44)];
        self.saveBtn = save;
...
-(UIButton *)backBtn{
    if (!_backBtn) {
        _backBtn = [[UIButton alloc]initWithFrame:CGRectMake(10,20, 60, 60)];
        //交给子类实现
...
-(UIButton *)jumpBtn{
    if (!_jumpBtn) {
        _jumpBtn = [[SPMyButtton alloc]initWithFrame:CGRectMake(20,SCREEN_H2-70, 40, 60)];
...
-(UIButton *)nextBtn{
    if (!_nextBtn) {
        _nextBtn = [[SPMyButtton alloc]initWithFrame:CGRectMake(SCREEN_W-80,SCREEN_H2-70, 40, 60)];
        //交给子类实现

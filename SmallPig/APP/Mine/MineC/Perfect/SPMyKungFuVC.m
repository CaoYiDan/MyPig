...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.chooseView loadWithCode:nil withAllArray:model2.subProperties haveChosedArr:model2.thirdLevelArr headerTitle:@"武功"];
...
-(UITableView *)tableView{
    if (!_tableView ) {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0,headerHeight, SCREEN_W, SCREEN_H2-headerHeight) style:UITableViewStyleGrouped];
...
-(SPMyHeader *)tableHeader{
    if (!_tableHeader) {
        _tableHeader = [[SPMyHeader alloc]initWithFrame:CGRectMake(0,0, SCREEN_W, headerHeight)];
        _tableHeader.backgroundColor = [UIColor whiteColor];
        _tableHeader.alpha = 0.9;
        [_tableHeader setImg:[UIImage imageNamed:@"grxx6_r1_c3"]];
        ////  + 新武功
...
-(void)next{
  ...
  NSString *skillSrt = [skillArr componentsJoinedByString:@","];
    ...
    [self postMessage:dict pushToVC:NSStringFromClass([SPMyTagVC class])];
}
